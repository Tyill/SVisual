...
namespace SV_TcpSrv {
	...
	int msts = 0;
#define SVCALL(func, mess) msts = func; \
 if (msts != 0){ if (server.errCBack) server.errCBack(std::string(mess) + " " + std::to_string(msts)); return; }
			...
			if (!client->outMess.empty()) {
				...
				int r = uv_write(&client->write_req,
								 ...
								 [](uv_write_t *req, int status) {
									 // все ок?
									 SVCALL(status, "on_read::uv_write error");
								 });
			}
		} else if (nread < 0) {
...
/// \param status
	void on_connect(uv_stream_t *server_handle, int sts) {
		...
		// все ок?
		SVCALL(sts, "on_connect:: error");

		client_t *client = new client_t();

		SVCALL(uv_tcp_init(server.uv_loop, &client->handle),
			 "on_connect::uv_tcp_init error");

		client->handle.data = client;

		SVCALL(uv_accept(server_handle, (uv_stream_t *) &client->handle),
			 "on_connect::uv_accept error");

		SVCALL(uv_read_start((uv_stream_t *) &client->handle, alloc_cb, on_read),
...
/// иниц-ия
	void initConnection() {
		...
		SVCALL(uv_tcp_init(server.uv_loop, &server.u_server),
			 "initConnection::uv_tcp_init error");

		SVCALL(uv_tcp_keepalive(&server.u_server, server.keepAlive, server.tout),
			 "initConnection::uv_tcp_keepalive error");

		struct sockaddr_in address;
		SVCALL(uv_ip4_addr(server.addr.c_str(), server.port, &address),
			 "initConnection::uv_ip4_addr error");

		SVCALL(uv_tcp_bind(&server.u_server, (const struct sockaddr *) &address, 0),
			 "initConnection::uv_tcp_bind error");

		SVCALL(uv_listen((uv_stream_t *) &server.u_server, 1000, on_connect),
			 "initConnection::uv_listen error");

		server.isRun = true;

		SVCALL(uv_run(server.uv_loop, UV_RUN_DEFAULT),
